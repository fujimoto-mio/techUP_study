...
CREATE TABLE `m_profiles` (
  ...
  `name` varchar(100) NOT NULL COMMENT '???O?A?ő??S?p100?????i200?o?C?g?j',
  `kana` varchar(100) NOT NULL COMMENT '?J?i?A?ő??S?p100?????i200?o?C?g?j',
  `age` int(11) NOT NULL COMMENT '?N??',
  `profile` text NOT NULL COMMENT '?v???t?B?[??',
  `place` int(5) NOT NULL COMMENT '?o?g?s???{???n?ԍ??i47?s???{??????ŊǗ??j',
  `sex` int(2) NOT NULL COMMENT '???ʁi1:?j?A2:???j',
  `created` datetime DEFAULT NULL COMMENT '?o?^???iYYYY-MM-DD HH:MM:SS?j',
  `modified` datetime DEFAULT NULL COMMENT '?X?V???iYYYY-MM-DD HH:MM:SS?j',
  PRIMARY KEY (`id`)
...
CREATE TABLE `m_users` (
  ...
  `tel` int(11) DEFAULT NULL COMMENT '?d?b?ԍ?',
...
-- Dump completed on 2024-07-14 23:13:30
